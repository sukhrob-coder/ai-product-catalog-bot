...
import config
import state
from ai_helper import analyze_catalog_product, analyze_product_query, generate_post_from_image
...
from poster import run_posting_job
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not _is_admin(update):
        return
    try:
        total = len(list_products(only_in_stock=False))
        in_stock = len(list_products(only_in_stock=True))
    except Exception as exc:
        await update.message.reply_text(f"DB xatosi: {exc}")
        return
    posted = len(state.load_posted_ids())
...
async def cmd_auto_post(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not _is_admin(update):
        await update.message.reply_text("Bu buyruq faqat admin uchun.")
        return
    await update.message.reply_text("DB'dagi navbatdagi mahsulot kanalga joylanmoqda...")
    await run_posting_job(context.bot)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "cancel_post":
        ...
        context.user_data.pop("admin_mode", None)
        await query.edit_message_caption(caption="❌ Post bekor qilindi.")
        return
    if not pending:
        context.user_data.pop("admin_mode", None)
        await query.edit_message_caption(caption="⚠️ Post topilmadi. Qaytadan rasm yuboring.")
        return
    if query.data == "publish_post":
        ...
        stream = io.BytesIO()
        await photo_file.download_to_memory(out=stream)
        pending["caption"] = await generate_post_from_image(stream.getvalue(), "image/jpeg", pending.get("user_note", ""))
        await query.edit_message_caption(caption="📝 <b>Yangi preview:</b>\n\n" + pending["caption"], parse_mode=ParseMode.HTML, reply_markup=_get_preview_keyboard())
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("holat", cmd_status))
    ...
    for value in config.POST_TIMES:
        hour, minute = map(int, value.split(":"))
        application.job_queue.run_daily(scheduled_job, time=dtime(hour=hour, minute=minute, tzinfo=tz), name=f"post_{value}")
